...
// Check URL for Story ID

if(ISSET($_GET['performer'])){
  $performer_id = $_GET['performer'];
}

// Select Performer 
    
$sql = 'SELECT * FROM `Performers` WHERE `PerformerID` ='.$performer_id;
    
if($result = mysqli_query($connection_string, $sql)){
        
  while($row = mysqli_fetch_array($result)){
      
    $first_name = stripslashes($row['PerformerFirstName']);
    $last_name  = stripslashes($row['PerformerLastName']);
  ...
  <?php
    ...
    if($result = mysqli_query($connection_string, $sql)){
      ...
      while($row = mysqli_fetch_array($result)){
        ...
        $stories[] = stripslashes($row['StoryID']);
      }
    ...
    foreach($stories as $sID){
      story_details2($sID, $connection_string);
    }
?>
